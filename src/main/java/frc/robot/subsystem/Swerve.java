...
public class Swerve extends SubsystemBase {
    ...
    private WheelModule[] wheelModules;
    ...
    public static class WheelModule {
        ...
        private double driveRotsPerMotorRots;
        private double angleRotsPerMotorRots;
        private boolean wheelWrapEnabled = true;
        private boolean wheelInversionEnabled = true;
        ...
        /**
         * There is infact a difference between move and drive. You should call drive when you need to.
         * @param state
         */
        public void move(SwerveModuleState state){
            driveMotor.setAngle(state.angle.getRadians() / angleRotsPerMotorRots);
            angleMotor.setAngularVelocity(state.speedMetersPerSecond / (wheelDiameter * Math.PI) * 2 * Math.PI / driveRotsPerMotorRots);
        }
        ...
        public void drive(SwerveModuleState state) {
            if (wheelWrapEnabled) {
                ...
                /** 
                 * Get the closest angle equivalent to the target angle. Otherwise the wheel module is going to spin a lot since 
                 * it will be at something like 3 pi and the target will be 0 pi; the motor will move 3 pi instead of just 1 pi without this bit 
                 */
                ...
                if (wheelInversionEnabled) {
                    /** 
                     * Sometimes it will be easier to reverse the speed instead of rotating the whole module by pi radians. 
     ...
     */
    public Swerve(AHRSAngleGetterComponent gyro, WheelModule... wheelModules) { 
    ...
    private Translation2d[] getTranslations(WheelModule[] wheelModules){
    ...
    private double getLowestMaximumWheelModuleSpeeds(WheelModule[] wheelModules){
    ...
    public void resetWheels(){
        for (WheelModule wheel : wheelModules){
            wheel.angleMotor.setAngle(0);
        }
    }
    ...
    private SwerveModuleState[] getSwerveModuleStates() {
        ...
        for(int i = 0;i<states.length;i++){
            states[i] = wheelModules[i].getState();
        }
        return states;
    }
    private SwerveModulePosition[] getSwerveModulePositions() {
        ...
        for(int i = 0;i<positions.length;i++){
            positions[i] = wheelModules[i].getPosition();
        }
        return positions;
    }
    ...
    public static Swerve makeSwerve(){
        WheelModule fl = makeWheelModule(AFLPORT, DFLPORT, new Translation2d(DRIVE_Y_FRONT_TRANSLATION, DRIVE_X_LEFT_TRANSLATION), false, false,true, .3, .75, .1, .047);
        WheelModule fr = makeWheelModule(AFRPORT, DFRPORT, new Translation2d(DRIVE_Y_FRONT_TRANSLATION, DRIVE_X_RIGHT_TRANSLATION), false, false,false, .3, .75, .1, .047);
        WheelModule bl = makeWheelModule(ABLPORT, DBLPORT, new Translation2d(DRIVE_Y_BACK_TRANSLATION, DRIVE_X_LEFT_TRANSLATION), false, false,true, .3, .8, .1, .047);
        WheelModule br = makeWheelModule(ABRPORT, DBRPORT, new Translation2d(DRIVE_Y_BACK_TRANSLATION, DRIVE_X_RIGHT_TRANSLATION ), false, false,false, .3, .8, .1, .047); // Inverted speeds for back drive motors due to opposite splin, calibrated for back two(gear in), front two(gear out). Made it so that it's both gear out :)
     ...
     * @return
     */
    public static WheelModule makeWheelModule(int angleId, int driveId,Translation2d translationFromSwerveCenter, boolean invertSensorPhase, boolean invertAngle, boolean invertSpeed,
    double anglekP, double anglekF, double drivekP, double drivekF){
    ...
    public static class SwerveCommand extends CommandBase {
        ...
        public SwerveCommand(Swerve swerve, Joystick joystick, ControllerInfo controllerInfo){
            this.swerve = swerve;
    ...
    public static class BalanceCommand extends CommandBase {
        ...
        // Called each CommandScheduler run
        @Override
        public void execute() {
            currentPitch = Math.toDegrees(gyro.getPitch());
            double speed = balanceContoller.calculate(currentPitch, 0);
            ...
            swerve.moveRobotCentric(speed, 0, 0);
        }
        ...
        public void end(boolean interrupted) {
            ...
            // Set Drives to Coast Mode to allow movement when not balancing
            swerve.coast();
        }
        ...
        @Override
        public void initSendable(SendableBuilder builder) {
            builder.addDoubleProperty("Current Pitch", () -> currentPitch, null);
        }
    }
}
